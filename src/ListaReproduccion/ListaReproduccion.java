...
public class ListaReproduccion implements IListaReproduccion {
	...
	public Map<IUsuario, List<ICancion>> generarListasPorUsuario(String rutaCSV) {
		...
		Map<IUsuario, List<ICancion>> resultado = new LinkedHashMap<>(); // Linked para que mantenga el orden de los
																			// usuarios
		...
		try (BufferedReader br = new BufferedReader(new FileReader(rutaCSV))) {
			...
			while ((linea = br.readLine()) != null) {
				if (linea.trim().isEmpty())
					continue; // ignorar línea vacía final
	...
	public List<ICancion> intercalarCanciones(Map<IUsuario, List<ICancion>> listasUsuario,
		...
		// Si no se pasan listas de canciones de cada usuario, se devuelve una lista
		// vacía.
		if (listasUsuario == null || listasUsuario.isEmpty()) {
			return cancionesIntercaladas;
		}

		// Mapa que mantiene el índice actual de cada usuario (cuántas canciones suyas
		// ya se han intercalado)
	...
	public List<Integer> generarListaReproduccionIntercalada(
		...
		// Este while se ejecuta cada vez que se recorren todas las canciones y hay que
		// volver a empezar
	...
	/**
	 * Filtra una lista de canciones según un rango de duración en segundos.
	 */
	@Override
	public List<ICancion> filtrarPorDuracion(List<ICancion> canciones, int minSeg,
			int maxSeg) {
		if (canciones == null)
			return List.of();
		return canciones.stream()
				.filter(c -> {
					int d = c.getDuracion();
					return d >= minSeg && d <= maxSeg;
				})
				.collect(Collectors.toList());
	}

	/**
	 * Ordena una lista de canciones por duración ("asc" o "desc").
	 */
	@Override
	public List<ICancion> ordenarPorDuracion(List<ICancion> canciones, String orden) {
		if (canciones == null)
			return List.of();
		List<ICancion> copia = new ArrayList<>(canciones);
		if ("asc".equalsIgnoreCase(orden)) {
			copia.sort(Comparator.comparingInt(ICancion::getDuracion));
		} else if ("desc".equalsIgnoreCase(orden)) {
			copia.sort(Comparator.comparingInt(ICancion::getDuracion).reversed());
		}
		return copia;
	}
